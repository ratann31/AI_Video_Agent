...
def load_pipeline_dependencies():
    """Load the ML and RAG modules only when an analysis is requested.

    Importing these modules initializes large AI libraries (Torch, Whisper, and
    sentence-transformers). Keeping them out of the initial page load lets the
    Streamlit interface render immediately.
    """
...
def validate_runtime(source: str, language: str) -> list[str]:
    issues = []

    if not os.getenv("MISTRAL_API_KEY"):
        issues.append("Missing MISTRAL_API_KEY in your .env file.")

    if language.lower() == "hinglish" and not os.getenv("SARVAM_API_KEY"):
        issues.append("Missing SARVAM_API_KEY for hinglish transcription.")

    if not is_url(source) and not os.path.exists(source):
        issues.append(f"Local file not found: {source}")
...
if run_btn:
    if not source.strip():
        ...
        issues = validate_runtime(source.strip(), language)
        if issues:
            for issue in issues:
                st.error(issue)
            st.stop()
        ...
        try:
            ...
            status.write("Loading AI components...")

            pipeline = load_pipeline_dependencies()
            status.write("AI components loaded.")

            update_step("audio", "active")
            status.write("Audio processing started...")
            chunks = pipeline["process_input"](source)
            update_step("audio", "done")
            status.write(f"Audio ready: {len(chunks)} chunk(s).")

            update_step("transcript", "active")
            status.write("Transcription started...")

            def on_chunk(index, total):
                status.write(f"Transcribing chunk {index}/{total}...")

            transcript = pipeline["transcribe_all"](chunks, language, on_chunk=on_chunk)
            update_step("transcript", "done")
            status.write("Transcription complete.")

            update_step("title", "active")
            status.write("Generating meeting title...")
            title = pipeline["generate_title"](transcript)
            update_step("title", "done")
            status.write("Title generated.")

            update_step("summary", "active")
            status.write("Building final summary...")
            summary = pipeline["summarize"](transcript)
            update_step("summary", "done")
            status.write("Summary completed.")

            update_step("extract", "active")
            status.write("Extracting action items, decisions, and open questions...")
            action_items = pipeline["extract_action_items"](transcript)
            decisions = pipeline["extract_key_decisions"](transcript)
            questions = pipeline["extract_questions"](transcript)
            update_step("extract", "done")
            status.write("Extraction completed.")

            update_step("rag", "active")
            status.write("Preparing RAG knowledge base...")
            rag_chain = pipeline["build_rag_chain"](transcript)
            update_step("rag", "done")
            status.write("RAG engine ready.")
...
if st.session_state.result:
    ...
    if send_btn and user_input.strip():
        with st.spinner("Thinking…"):
            answer = load_pipeline_dependencies()["ask_question"](
                r["rag_chain"], user_input.strip()
            )
